...
public class PlayerController : MonoBehaviour
{
    ...
    private PlayerState currentState = PlayerState.walk; // Stan gracza
    ...
    {
        animator.SetBool("attacking", true);
        currentState = PlayerState.attack; // Zmiana stanu na atak
        yield return null;
        animator.SetBool("attacking", false);
        yield return new WaitForSeconds(.3f);
        currentState = PlayerState.walk; // Powrót do stanu chodzenia
    }
    void Interact()
    {
        // Ustalanie kierunku interakcji
